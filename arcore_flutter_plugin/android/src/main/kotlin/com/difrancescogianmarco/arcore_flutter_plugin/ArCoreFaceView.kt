...
import io.flutter.plugin.common.EventChannel.StreamHandler
 ...
 class SpeechResultStreamHandler(): EventChannel.StreamHandler {
      ...
      public fun sendIt(x: DoubleArray)
      { 
         this._eventSink?.success(x)
      }
}

class ArCoreFaceView(activity:Activity,context: Context, messenger: BinaryMessenger, id: Int, debug: Boolean) : 
BaseArCoreView(activity, context, messenger, id, debug) {
    ...
    val eventChannel = EventChannel(messenger, "events")
            ...
            run {
                ...
                faceList?.let {
                    ...
                    for (face in faceList) {
                        var p = face.getCenterPose()
                        var rotation = p.getRotationQuaternion()
                        var translation = p.getTranslation()
                        val x = doubleArrayOf(translation.get(0).toDouble() , + translation.get(1).toDouble() , translation.get(2).toDouble()
                        
                        , rotation.get(0).toDouble(), rotation.get(1).toDouble(), rotation.get(2).toDouble(),rotation.get(3).toDouble()
                        )

                   
                        // Log.d("TAG", " " + x.size + " " + x.size)
                        streamHandler.sendIt(x)
